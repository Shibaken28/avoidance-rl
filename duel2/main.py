...
class QNet(nn.Module):
    def __init__(self, num_actions):
        ...
        self.fc_val2 = nn.Linear(256, 32)
        self.fc_val3 = nn.Linear(32, 1)
        # Dueling Network
        self.fc_adv1 = nn.Linear(self.feature_size(), 256)
        self.fc_adv2 = nn.Linear(256, 32)
        self.fc_adv3 = nn.Linear(32, num_actions)
    ...
    def forward(self, x):
        ...
        val = F.relu(self.fc_val2(val))
        val = self.fc_val3(val)

        adv = F.relu(self.fc_adv1(x))
        adv = F.relu(self.fc_adv2(adv))
        adv = self.fc_adv3(adv)
