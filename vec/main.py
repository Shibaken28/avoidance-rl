...
class Game:
    ...
    def states(self):
        ...
        for enemy in self.enemy_list:
            ret.append(enemy.x) #/self.width)
            ret.append(enemy.y) #/self.height)
        ret.append(self.player_pos[0]) #/self.width)
        ret.append(self.player_pos[1]) #/self.height)
...
class QNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)  # フラットな形状に変換
        x = F.relu(self.fc1(x))
        return self.fc2(x)
